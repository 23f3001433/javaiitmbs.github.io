...
function highlightSyntax(line) {
  ...
  if (escapedLine.includes('//')) {
    ...
    return highlightCodePart(beforeComment) + '<span class="comment">' + comment + '</span>';
  } else {
...
function highlightCodePart(text) {
  ...
  const keywords = [
    ...
    'interface', 'extends', 'implements', 'abstract', 'new', 'return',
    'if', 'else', 'for', 'while', 'do', 'switch', 'case', 'break',
    'continue', 'try', 'catch', 'finally', 'throw', 'throws', 'import',
    'package', 'enum', 'null', 'true', 'false', 'this', 'super'
  ];
  ...
  result = result.replace(/\b([A-Z][a-zA-Z0-9_]*)\b/g, function(match, className) {
  ...
  // Function calls
  result = result.replace(/\b([a-zA-Z][a-zA-Z0-9_]*)\s*\(/g, function(match, funcName) {
